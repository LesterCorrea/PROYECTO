...
@section('page-title', 'Editar carrusel')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        <form method="POST"
            action="{{ route('librarian.carruseles.update', $carrusele) }}"
            class="space-y-6">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $carrusele->title) }}" required
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                    class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('description', $carrusele->description) }}</textarea>
            </div>

            {{-- Tipo de elementos --}}
            <div x-data="{ selected: '{{ old('type', $carrusele->type) }}' }">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    <label class="flex items-center gap-2.5 p-3 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <input type="checkbox" name="sections[]" value="{{ $value }}"
                            {{ in_array($value, old('sections', $carrusele->sections ?? [])) ? 'checked' : '' }}
            ...
            <div class="flex items-center gap-4">
                <div class="w-32">
                    ...
                    <input type="number" name="order" value="{{ old('order', $carrusele->order) }}" min="0"
                        class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div class="flex items-center gap-2 mt-5">
                    <input type="checkbox" name="is_active" id="is_active" value="1"
                        {{ old('is_active', $carrusele->is_active) ? 'checked' : '' }}
                        class="w-4 h-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500" />
