...
@section('title', 'Añadir Revista')
@section('page-title', 'Añadir revista')
...
<div class="max-w-3xl"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        <form method="POST"
            action="{{ route('librarian.revistas.store') }}"
            ...
            {{-- Autores múltiples con creación inline --}}
            <div x-data="{ authors: [{ id: '', role: 'Autor' }] }">
                ...
                <div class="space-y-2" @author-created.window="
                    ...
                    if (!last.id) last.id = $event.detail.id;
                    else authors.push({ id: $event.detail.id, role: 'Autor' });
                ">
                    <template x-for="(author, index) in authors" :key="index">
                        <div class="flex gap-2 items-center">
                            <select :name="`author_ids[${index}]`" required
                                x-model="author.id"
                                :id="`author_select_${index}`"
            ...
            {{-- Copias disponibles --}}
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-0">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                <div>
                    ...
                    <input type="number" name="issue_number" value="{{ old('issue_number') }}" min="1"
                        class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
            ...
            <div x-data="{ preview: null }">
                ...
                <div x-show="preview" class="mt-2 flex items-center gap-3">
                    ...
                    <button type="button"
                        @click="preview = null; $refs.coverInput.value = ''"
...
<script>
    function bookForm(config) {
        return {
            ...
            // Autor tiene lógica especial: añade al select dinámico y emite evento
            async submitAuthor() {
                ...
                try {
                    ...
                    // Añadir el nuevo autor a todos los selects de autores
                    ...
                    // Emitir evento para que Alpine asigne el nuevo autor
            ...
            // Categoría y editorial: añaden opción a su select y auto-seleccionan
