...
<div class="max-w-lg">
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        <form method="POST" action="{{ route('librarian.categorias.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="text" id="color-text" value="{{ old('color', '#6366F1') }}"
            ...
            <div>
                ...
                <select name="type" required
                    class="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="book" {{ old('type') === 'book'     ? 'selected' : '' }}>Solo libros</option>
                    <option value="magazine" {{ old('type') === 'magazine' ? 'selected' : '' }}>Solo revistas</option>
...
<script>
    ...
    const hiddenColor = document.querySelector('input[name="color"]');
